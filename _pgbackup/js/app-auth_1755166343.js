...
// --- app-auth.js (top) -------------------------------------------
// Safety: ensure RBAC exists even if rbac.js didn't load for some reason.
window.RBAC = window.RBAC || (function () {
  ...
  const set = (s) => {
    ...
    window.RBAC._session = s;
  };
  const clear = () => {
    try { sessionStorage.removeItem(KEY); } catch {}
    window.RBAC._session = null;
  };
  return { session: get, setSession: set, clearSession: clear, _session: get() };
})();
// -----------------------------------------------------------------
